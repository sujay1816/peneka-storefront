...
// Send restock emails to all customers waiting for this product+colour+size
// Uses Resend directly (not a self-HTTP-call) so it works even when NEXT_PUBLIC_SITE_URL is unset.
async function notifyRestockWaiters(supabase: any, productId: string, colour: string, size: string) {
  try {
    ...
    for (const req of requests) {
      ...
      await resend.emails.send({
        ...
        html: restockAvailableEmailHtml(productName, colour, size, productUrl, brandName),
      }).catch(() => {})
    }
...
function restockAvailableEmailHtml(productName: string, colour: string, size: string, productUrl: string, brandName: string): string {
  return `<!DOCTYPE html><html>
<body style="margin:0;padding:0;background:#FBF0DA;font-family:'Karla',Arial,sans-serif;">
  <div style="max-width:560px;margin:32px auto;background:white;border-radius:8px;overflow:hidden;">
    <div style="background:linear-gradient(135deg,#9A3324,#7A2A1D);padding:28px 32px;text-align:center;">
      <h1 style="color:white;font-size:22px;font-weight:300;margin:0;font-family:Georgia,serif;">${brandName}</h1>
      <p style="color:#DDA119;font-size:10px;letter-spacing:4px;text-transform:uppercase;margin:4px 0 0;">✦ WEAR THE EPICS ✦</p>
    </div>
    <div style="padding:32px;">
      ...
      <p style="font-size:14px;color:#5A4A3A;line-height:1.7;margin:0 0 20px;text-align:center;">The tee you were waiting for is back in stock. Grab it before it sells out!</p>
      <div style="background:#FFF8F0;border-left:3px solid #DDA119;padding:14px 18px;margin-bottom:24px;">
        ...
        <p style="margin:4px 0 0;font-size:13px;color:#5A4A3A;">Colour: ${colour} &nbsp;·&nbsp; Size: ${size}</p>
      </div>
      <div style="text-align:center;margin-bottom:24px;">
        <a href="${productUrl}" style="display:inline-block;padding:14px 32px;background:linear-gradient(135deg,#9A3324,#7A2A1D);color:white;text-decoration:none;font-size:13px;font-weight:500;letter-spacing:0.1em;text-transform:uppercase;border-radius:4px;">Shop Now &#8594;</a>
      </div>
      <p style="font-size:12px;color:#9A8A7A;text-align:center;margin:0;">Hurry &#8212; limited stock available!</p>
    </div>
    <div style="background:#2A190E;padding:16px 32px;text-align:center;">
  ...
  try {
    ...
    if (type === 'deduct') {
      for (const item of items) {
        const { error } = await supabase.rpc('deduct_stock', {
          p_product_id: item.product_id, p_colour: item.colour, p_size: item.size || 'M', p_quantity: item.quantity,
        })
    ...
    if (type === 'restore') {
      for (const item of items) {
        ...
        notifyRestockWaiters(supabase, item.product_id, item.colour, item.size || 'M').catch(() => {})
      }
      return NextResponse.json({ success: true })
    }
