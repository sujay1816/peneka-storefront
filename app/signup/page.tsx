...
function SignupForm() {
  ...
  if (emailSent) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#111111', fontFamily: 'var(--font-body)' }}>
        <div style={{ width: '100%', maxWidth: 400, textAlign: 'center', padding: 32 }}>
          <div style={{ fontSize: 56, marginBottom: 20 }}>📧</div>
          <h2 style={{ fontSize: 28, fontWeight: 300, color: 'white', marginBottom: 12, fontFamily: 'var(--font-heading)' }}>
            Check your inbox
          </h2>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', background: '#0D0D0D', fontFamily: 'var(--font-body)' }}>
      ...
      <div className="hidden lg:flex" style={{ width: '50%', position: 'relative', overflow: 'hidden', background: 'linear-gradient(160deg, #0A1A0E 0%, #0F2C18 35%, #1B6B35 60%, #0F2C18 85%, #0A1A0E 100%)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', padding: 48, textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: 48, fontWeight: 300, color: 'white', marginBottom: 8, fontFamily: 'var(--font-heading)' }}>Join the Legacy</h1>
          ...
          <p style={{ fontSize: 18, fontWeight: 300, fontStyle: 'italic', color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-voice)', lineHeight: 1.6 }}>
            "Create your account and<br />wear the epics"
          </p>
          <div style={{ marginTop: 36, textAlign: 'left', width: '100%', maxWidth: 260 }}>
            {['Early access to new arrivals', 'Exclusive member discounts', 'Order tracking & history', 'Wishlist across all devices'].map((b, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <div style={{ width: 20, height: 20, borderRadius: '50%', background: 'rgba(221,161,25,0.2)', border: '1px solid rgba(221,161,25,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(16px, 5vw, 32px)', background: '#111111', position: 'relative', minHeight: '100svh' }}>
        ...
        <div style={{ width: '100%', maxWidth: 360, paddingTop: 40 }}>
          ...
          <h2 style={{ fontSize: 30, fontWeight: 300, color: 'white', marginBottom: 8, fontFamily: 'var(--font-heading)' }}>Create account</h2>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', marginBottom: 28 }}>Join thousands wearing the epics</p>

          <button type="button" onClick={handleGoogle} disabled={googleLoading}
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '13px 16px', marginBottom: 20, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: 'white', fontSize: 14, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
          ...
          <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              ...
              <div key={f.label}>
                ...
                <input type={f.type} value={f.value} onChange={e => f.set(e.target.value)} required placeholder={f.placeholder} autoComplete={(f as any).ac}
                  style={{ width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: 'white', fontSize: 16, outline: 'none', fontFamily: 'var(--font-body)' }} onFocus={e => e.currentTarget.style.boxShadow='0 0 0 2px #DDA119'} onBlur={e => e.currentTarget.style.boxShadow='none'} />
              </div>
            ))}
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required placeholder="Min. 8 characters" autoComplete="new-password"
                  style={{ width: '100%', padding: '12px 44px 12px 16px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: 'white', fontSize: 14, outline: 'none', fontFamily: 'var(--font-body)' }} />
            ...
            <button type="submit" disabled={loading}
              style={{ width: '100%', padding: 13, background: loading ? 'rgba(193,68,47,0.5)' : 'linear-gradient(135deg, #C1442F 0%, #7A2A1D 100%)', color: 'white', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'var(--font-body)', boxShadow: '0 4px 20px rgba(193,68,47,0.4)' }}>
              {loading
